...
class Common_util_pub {
    ...
    function postXmlSSLCurl($xml, $url, $second = 30) {
        ...
        curl_setopt($ch, CURLOPT_CAINFO, WxPayConf_pub::CAINFO_PATH); // CA根证书（用来验证的网站证书是否是CA颁布）
        //post提交方式
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
        $data = curl_exec($ch);
 ...
 * 企业支付接口类
 */
class Mch_pay_pub extends Wxpay_client_pub {
    ...
    function createXml() {
        try {
            ...
            if ($this->parameters["partner_trade_no"] == null) {
                throw new SDKRuntimeException("缺少企业支付接口必填参数partner_trade_no！" . "<br>");
            } elseif ($this->parameters["desc"] == null) {
                throw new SDKRuntimeException("缺少企业支付接口必填参数desc！" . "<br>");
            } elseif ($this->parameters["amount"] == null) {
                throw new SDKRuntimeException("缺少企业支付接口必填参数amount！" . "<br>");
            } elseif ($this->parameters["check_name"] == null) {
                throw new SDKRuntimeException("缺少企业支付接口必填参数check_name！" . "<br>");
            } elseif (
                (
                    $this->parameters["check_name"] == "FORCE_CHECK" 
                    || $this->parameters["check_name"] == "OPTION_CHECK"
                ) 
                && $this->parameters["re_user_name"] == NULL
            ) {
                throw new SDKRuntimeException("企业支付接口中，缺少必填参数re_user_name！" . "<br>");
            }
            $this->parameters["mch_appid"] = WxPayConf_pub::APPID; //公众账号ID
            $this->parameters["mchid"] = WxPayConf_pub::MCHID; //商户号
            $this->parameters["spbill_create_ip"] = '123.57.210.1'; //终端ip	    
            $this->parameters["nonce_str"] = $this->createNoncestr(); //随机字符串
            $this->parameters["sign"] = $this->getSign($this->parameters); //签名
